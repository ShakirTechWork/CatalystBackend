...
export const updateAdminData = asyncHandler(async (req: Request, res: Response) => {
    const { currentUserMongoObjectId, admin } = req.body;

    if (!isValidMongoObjectId(currentUserMongoObjectId)) {
    ...
    const objectId = new Types.ObjectId(String(currentUserMongoObjectId));
